...
import { HpfFrequencyAction } from '../../src/validators/index.js'
...
describe('hpfFrequency action', () => {
	...
	describe.each(testInputs)('input %s', (input) => {
		it.each(testFrequencies)('frequency=%s', (frequency) => {
			const hpfFrequencyAction: HpfFrequencyAction = {
				...baseAction,
				options: {
					input,
					frequency,
			...
			void moduleInstance.actionDefinitions.hpfFrequency?.callback?.(
				hpfFrequencyAction as CompanionActionEvent,
