...
import { SetSocketPreampPadAction } from '../../src/validators/index.js'
...
describe('setSocketPreampPad action', () => {
	...
	const testPadStates = [true, false]
	...
	describe.each(testCases)('socket type %s', (socketType, socketCount) => {
		...
		describe.each(testSockets)('socket %s', (socketNo: number) => {
			it.each(testPadStates)('pad=%s', (pad) => {
				const socketIndex = socketNo - 1

				const setSocketPreampPadAction: SetSocketPreampPadAction = {
					...baseAction,
					options: {
						...baseAction.options,
						socketType,
				...
				void moduleInstance.actionDefinitions.setSocketPreampPad?.callback?.(
					setSocketPreampPadAction as CompanionActionEvent,
