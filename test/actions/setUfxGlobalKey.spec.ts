...
import { SetUfxGlobalKeyAction } from '../../src/validators/index.js'
...
describe('setUfxGlobalKey action', () => {
	...
	// Test first, middle, and last key
	const testKeys: [string, number][] = [
		['C', 0x00],
		['F#', 0x06],
		['B', 0x0b],
	]

	it.each(testKeys)('key=%s', (_keyName, key) => {
		const setUfxGlobalKeyAction: SetUfxGlobalKeyAction = {
			...baseAction,
			options: {
				key,
			},
		...
		void moduleInstance.actionDefinitions.setUfxGlobalKey?.callback?.(
			setUfxGlobalKeyAction as CompanionActionEvent,
			{} as CompanionActionContext,
		)

		expect(sendMidiToDliveSpy).toHaveBeenCalledTimes(1)
		expect(sendMidiToDliveSpy).toHaveBeenCalledWith([0xb0, 0x0c, key])
	})
})
